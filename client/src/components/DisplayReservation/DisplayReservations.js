...
const DisplayReservation = () => {
  ...
  useEffect(() => {
    const displayData = async () => {
      try {
        ...
        const res = await axios.get(`http://localhost:5001/displayreservation/${createdBy._id}`);
  ...
  const handleDelete = async (id) => {
    try {
      ...
      const createdBy = JSON.parse(localStorage.getItem("user"));
      const afterDeleteData = await axios.get(`http://localhost:5001/displayreservation/${createdBy._id}`);
